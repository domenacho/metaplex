...
async function mintV2(keypair, env, candyMachineAddress, rpcUrl) {
    ...
    if (collectionPDAAccount) {
        try {
            const collectionMint = (await anchorProgram.account.collectionPda.fetch(collectionPDA));
            const collectionAuthorityRecord = (await (0, accounts_1.getCollectionAuthorityRecordPDA)(collectionMint.mint, collectionPDA))[0];
            if (collectionMint) {
                const collectionMetadata = await (0, accounts_1.getMetadata)(collectionMint.mint);
                const collectionMasterEdition = await (0, accounts_1.getMasterEdition)(collectionMint.mint);
                remainingAccounts.push(...[
                    {
